...
def telethon_session():
    ...
    # logging in
    try:
        with TelegramClient(StringSession(), API_ID, API_HASH) as Arank:
            print("Generating a string session for •Arank•")
            try:
                Arank.send_message(
                    "me",
        ...
        print(er)
        print("If you think It as a Bug, Report to @Carding_Chronicle.\n\n")
...
def pyro_session():
    ...
    try:
        with Client(name="Arank", api_id=API_ID, api_hash=API_HASH, in_memory=True) as pyro:
            ss = pyro.export_session_string()
            pyro.send_message(
                "me",
                f"`{ss}`\n\nAbove is your Pyrogram Session String for @TheArank. **DO NOT SHARE it.**",
            )
...
def main():
    ...
    try:
        type_of_ss = int(
            input(
                "\nArank supports both telethon as well as pyrogram sessions.\n\nWhich session do you want to generate?\n1. Telethon Session.\n2. Pyrogram Session.\n\nEnter choice:  "
            )
        )
