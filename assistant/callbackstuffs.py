...
try:
    from pyArank.fns.gDrive import GDriveManager
except ImportError:
    GDriveManager = None
...
from pyArank.fns.helper import fast_download, progress
from pyArank.fns.tools import Carbon, async_searcher, get_paste, telegraph_client
from pyArank.startup.loader import Loader
...
_buttons = {
    "otvars": {
        "text": "Other Variables to set for @TeamArank:",
        "buttons": [
            [
    ...
    "oofdm": {
        "text": "About [Dual Mode](https://t.me/Mr_Mrs_Krishna)",
        "buttons": [
            [
...
async def update(eve):
    ...
    if heroku_api:
        ...
        execl(sys.executable, sys.executable, "-m", "pyArank")
...
async def changes(okk):
    ...
    if not match:
        try:
            ...
            return await okk.edit(
                f"**• Arank Userbot •**{cli}", file=img, buttons=button
            )
        except Exception as er:
            LOGS.exception(er)
    ...
    if len(changelog_str) > 1024:
        ...
        with open("Arank_updates.txt", "w+") as file:
            file.write(tl_chnglog)
        await okk.edit(
            get_string("upd_5"),
            file="Arank_updates.txt",
            buttons=button,
        )
        remove("Arank_updates.txt")
        return
    await okk.edit(
        changelog_str,
        buttons=button,
...
async def _(e):
    ...
    async with asst.conversation(e.sender_id) as conv:
        ...
        if GDrive._create_token_file(code=code.text):
            await conv.send_message(
                "`Success!\nYou are all set to use Google Drive with Arank Userbot.`",
...
async def _(e):
    if not e.is_private:
        return
    await e.edit(
        "Click Authorise and send the code.\n\nYou can use your own CLIENT ID and SECRET by [this](https://t.me/Mr_Mrs_Krishna)",
        buttons=[
            [
