...
from pyArank._misc import SUDO_M, owner_and_sudos
...
**Arank [v{Arank_version}](https://github.com/CoderXKrishna/Arank), powered by @Mr&MrsHitlerrr**
"""
...
async def own(event):
    msg = Owner_info_msg.format(
        mention=event.sender.mention, me=inline_mention(Arank_bot.me)
    )
    if custom_info:
        msg += "\n\n• Powered by **@TeamArank**"
    await event.edit(
        msg,
...
async def Arank(event):
    ...
    if event.sender_id not in SUDO_M.fullsudos:
        ok = ""
        me = inline_mention(Arank_bot.me)
        ...
        if not udB.get_key("STARTMSG"):
            ...
            await event.reply(
                f"Hey there {mention}, this is Arank Assistant of {me}!\n\n{ok}",
