...
async def autoupdate_local_database():
    from .. import Var, asst, udB, Arank_bot

    global db_url
    db_url = (
        udB.get_key("TGDB_URL") or Var.TGDB_URL or Arank_bot._cache.get("TGDB_URL")
    )
    if db_url:
        _split = db_url.split("/")
        _channel = _split[-2]
...
async def startup_stuff():
    ...
    if CT:
        ...
        ARAConfig.thumb = path
        try:
            await download_file(CT, path)
        except Exception as er:
            LOGS.exception(er)
    elif CT is False:
        ARAConfig.thumb = None
    ...
    if TZ and timezone:
        try:
            ...
            LOGS.critical(
                "Incorrect Timezone ,\nCheck Available Timezone From Here https://graph.org/Arank-02-18-3So Time is Default UTC"
            )
            os.environ["TZ"] = "UTC"
            time.tzset()
...
async def autopilot():
    ...
    if not channel:
        ...
        async def _save(exc):
            udB._cache["LOG_CHANNEL"] = Arank_bot.me.id
            await asst.send_message(
                Arank_bot.me.id, f"Failed to Create Log Channel due to {exc}.."
            )
    ...
    if isinstance(chat.photo, ChatPhotoEmpty):
        photo, _ = await download_file(
            "https://graph.org/file/0b9d7bd278272147f1f3c.jpg", "channelphoto.jpg"
        )
...
async def fetch_ann():
    ...
    try:
        updts = await async_searcher(
            "https://Arank-api.vercel.app/announcements", post=True, re_json=True
        )
        for upt in updts:
            key = list(upt.keys())[0]
            if key not in get_:
...
• Here, are the Some Basic stuff from, where you can Know, about its Usage."""
        PHOTO = "https://telegra.ph/file/0b9d7bd278272147f1f3c.jpg"
