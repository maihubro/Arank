...
REPO="https://github.com/CoderXKrishna/Arank.git"
CURRENT_DIR="$(pwd)"
ENV_FILE_PATH=".env"
DIR="/root/CoderXKrishna"

while [ $# -gt 0 ]; do
    case "$1" in
    --dir=*)
        DIR="${1#*=}" || DIR="/root/CoderXKrishna"
        ;;
    --branch=*)
...
clone_repo() {
    # check if pyArank, startup, plugins folders exist
    cd $DIR
    if [ -d $DIR ]; then
        if [ -d $DIR/.git ]; then
            echo -e "Updating Arank ${BRANCH}... "
            cd $DIR
            git pull
...
misc_install() {
    ...
    if [ $OKTETO_TOKEN ]; then
        ...
        if [ -d $DIR/vcbot ]; then
            ...
            git clone https://github.com/CoderXKrishna/VcBot $DIR/vcbot
        fi
...
main() {
    echo -e "Starting Arank Setup..."
    if [ -d "pyArank" ] && [ -d "resources" ] && [ -d "plugins" ]; then
        DIR=$CURRENT_DIR
    fi
    if [ -f $ENV_FILE_PATH ]
    then
