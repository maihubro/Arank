...
from . import Redis, eor, get_string, udB, Arank_cmd


@Arank_cmd(pattern="setdb( (.*)|$)", fullsudo=True)
async def _(ult):
...
@Arank_cmd(pattern="deldb( (.*)|$)", fullsudo=True)
async def _(ult):
...
@Arank_cmd(pattern="rendb( (.*)|$)", fullsudo=True)
async def _(ult):
    match = ult.pattern_match.group(1).strip()
    if not match:
