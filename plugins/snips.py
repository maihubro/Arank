...
from pyArank._misc import sudoers
from pyArank.dB.snips_db import add_snip, get_snips, list_snip, rem_snip
from pyArank.fns.tools import create_tl_btn, format_btn, get_msg_button

from . import events, get_string, mediainfo, udB, Arank_bot, Arank_cmd
from ._inline import something


@Arank_cmd(pattern="addsnip( (.*)|$)")
async def an(e):
    ...
    Arank_bot.add_handler(add_snips, events.NewMessage())


@Arank_cmd(pattern="remsnip( (.*)|$)")
async def rs(e):
    wrd = (e.pattern_match.group(1).strip()).lower()
    if not wrd:
    ...
    for z in xx:
        if k := get_snips(z):
            ...
            if rep:
                ...
                await Arank_bot.send_message(e.chat_id, msg, file=media)
