...
from pyArank.dB.filestore_db import del_stored, get_stored_msg, list_all_stored_msgs
from pyArank.fns.tools import get_file_link

from . import HNDLR, asst, get_string, in_pattern, udB, Arank_bot, Arank_cmd


@Arank_cmd(pattern="store$")
...
async def _(event):
    ...
    del_stored(match[1])
    await Arank_bot.delete_messages(udB.get_key("LOG_CHANNEL"), int(msg_id))
    await event.eor("__Deleted__")


@Arank_cmd("liststored$")
