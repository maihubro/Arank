...
from pyArank.dB import stickers
from pyArank.dB.echo_db import check_echo
from pyArank.dB.forcesub_db import get_forcesetting
from pyArank.dB.gban_mute_db import is_gbanned
from pyArank.dB.greetings_db import get_goodbye, get_welcome, must_thank
from pyArank.dB.nsfw_db import is_profan
from pyArank.fns.helper import inline_mention
from pyArank.fns.tools import async_searcher, create_tl_btn, get_chatbot_reply

try:
    from ProfanityDetector import detector
except ImportError:
    detector = None
from . import LOG_CHANNEL, LOGS, asst, get_string, types, udB, Arank_bot
from ._inline import something


@Arank_bot.on(events.ChatAction())
...
async def DummyHandler(ult):
    ...
    if (
        ...
        if not user.bot:
            ...
            try:
                await Arank_bot(GetParticipantRequest(int(joinchat), user.id))
            except UserNotParticipantError:
                await Arank_bot.edit_permissions(
                    ult.chat_id, user.id, send_messages=False
                )
                res = await Arank_bot.inline_query(
    ...
    if ult.user_joined or ult.added_by:
        ...
        # gbans and @ArankBans checks
        if udB.get_key("Arank_BANS"):
            try:
                is_banned = await async_searcher(
                    "https://bans.Arank.tech/api/status",
                    json={"userId": user.id},
                    post=True,
                    re_json=True,
                )
                if is_banned["is_banned"]:
                    ...
                    await ult.respond(
                        f'**@ArankBans:** Banned user detected and banned!\n`{str(is_banned)}`.\nBan reason: {is_banned["reason"]}',
                    )
...
@Arank_bot.on(events.NewMessage(incoming=True))
...
@Arank_bot.on(events.Raw(types.UpdateUserName))
