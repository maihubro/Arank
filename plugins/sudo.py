...
from . import get_string, inline_mention, udB, Arank_bot, Arank_cmd


@Arank_cmd(pattern="addsudo( (.*)|$)", fullsudo=True)
async def _(ult):
...
@Arank_cmd(pattern="delsudo( (.*)|$)", fullsudo=True)
async def _(ult):
    inputs = ult.pattern_match.group(1).strip()
    if ult.reply_to_msg_id:
        replied_to = await ult.get_reply_message()
...
async def _(ult):
    ...
    if not m:
        m = "[False](https://graph.org/Arank-02-18-2)"
