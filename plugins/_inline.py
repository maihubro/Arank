...
from pyArank._misc._assistant import callback, in_pattern
from pyArank.dB._core import HELP, LIST
from pyArank.fns.helper import gen_chlog, time_formatter, updater
from pyArank.fns.misc import split_list
...
SUP_BUTTONS = [
    [
        Button.url("• Repo •", url="https://github.com/CoderXKrishna/Arank"),
        Button.url("• Support •", url="t.me/ArankSupportChat"),
    ],
]
...
async def inline_alive(o):
    TLINK = inline_pic() or "https://telegra.ph/file/0b9d7bd278272147f1f3c.jpg"
    MSG = "• **Arank Userbot •**"
...
async def inline_handler(event):
    ...
    if inline_pic():
        ...
        result = await event.builder.article(
            title="Arank Help Menu", text=text, buttons=_main_help_menu
        )
...
async def ibuild(e):
    ...
    if pic:
        try:
            ...
            LOGS.exception(er)
    result = [
        await builder.article("Arank Op", text=txt, link_preview=False, buttons=btn)
    ]
