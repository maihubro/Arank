...
from pyArank._misc._assistant import asst_cmd
from pyArank.dB.gban_mute_db import is_gbanned
from pyArank.fns.tools import get_chat_and_msgid
...
from pyArank.fns.info import get_chat_info
...
@Arank_cmd(pattern="kickme$", fullsudo=True)
async def leave(ult):
...
@Arank_cmd(pattern="paste( (.*)|$)", manager=True, allow_all=True)
async def _(event):
    try:
...
async def telegraphcmd(event):
    xx = await event.eor(get_string("com_1"))
    match = event.pattern_match.group(1).strip() or "Arank"
    reply = await event.get_reply_message()
    if not reply:
...
@Arank_cmd(pattern="suggest( (.*)|$)", manager=True)
