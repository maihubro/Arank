...
from pyArank.fns.admins import ban_time
from pyArank.fns.info import get_uinfo

from . import HNDLR, LOGS, eod, eor, get_string, inline_mention, types, Arank_cmd
...
@Arank_cmd(pattern="pin$", manager=True, require="pin_messages", fullsudo=True)
async def pin(msg):
    if not msg.is_reply:
...
@Arank_cmd(pattern="purge( (.*)|$)", manager=True, require="delete_messages")
...
@Arank_cmd(pattern="pinned", manager=True, groups_only=True)
