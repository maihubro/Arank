...
from pyArank.fns.helper import download_file
from pyArank.fns.tools import get_google_images

from . import LOGS, get_help, get_string, udB, Arank_bot, Arank_cmd

__doc__ = get_help("help_autopic")


@Arank_cmd(pattern="autopic( (.*)|$)")
async def autopic(e):
...
if search := udB.get_key("AUTOPIC"):
    ...
    async def autopic_func():
        ...
        file = await Arank_bot.upload_file(filee)
        await Arank_bot(UploadProfilePhotoRequest(file))
        os.remove(filee)
