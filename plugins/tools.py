...
from pyArank.fns.tools import metadata, translate
...
from . import inline_mention, is_url_ok, json_parser, mediainfo, Arank_cmd


@Arank_cmd(pattern="tr( (.*)|$)", manager=True)
async def _(event):
...
@Arank_cmd(pattern="bots( (.*)|$)", groups_only=True, manager=True)
async def _(ult):
...
async def _(e):
    ...
    try:
        ...
        with io.BytesIO(str.encode(text)) as out_file:
            ...
            await e.reply(f"`{e.text}`", file=out_file, thumb=ARAConfig.thumb)
        await e.delete()
