...
from . import check_filename, get_string, Arank_bot, Arank_cmd


@Arank_cmd(pattern="qrcode( (.*)|$)")
async def cd(e):
    ...
    default, cimg = ARAConfig.thumb, None
    if reply and (reply.sticker or reply.photo):
        cimg = await reply.download_media()
    elif Arank_bot.me.photo and not Arank_bot.me.photo.has_video:
        cimg = (await e.client.get_profile_photos(Arank_bot.uid, limit=1))[0]
